...
- 자동 보정(사진 회전 보정 · 흑백 변환 · 대비 조정 · 작은 이미지 확대)
  - 글자가 작을수록 인식률이 떨어지므로 폭 2400px까지 키워서 넘깁니다
...
- **신뢰도**: 결과에 평균 신뢰도가 표시됩니다. 다만 이 값은 Tesseract가
  *읽어낸 단어에 대해서만* 계산되어, 아예 놓친 글자는 점수에 반영되지
  않습니다. 신뢰도가 높아도 텍스트가 빠져 있을 수 있으니 품질 판단은
  숫자가 아니라 결과 텍스트를 원본과 대조해서 하세요.
